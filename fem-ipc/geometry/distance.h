...
point_edge_dist_type(const Eigen::Vector3d &p, const Eigen::Vector3d &e0, const Eigen::Vector3d &e1) {
    Eigen::Vector3d edge = e1 - e0;
    const double edge_length_square = edge.squaredNorm();
    if (edge_length_square == 0) { return PointEdgeDistType::P_E0; }

    const double ratio = edge.dot(p - e0) / edge_length_square;
    if (ratio < 0) {
...
static PointTriangleDistType point_triangle_dist_type(
    const Eigen::Vector3d &p, const Eigen::Vector3d &t0, const Eigen::Vector3d &t1, const Eigen::Vector3d &t2) {
    const Eigen::Vector3d normal = (t1 - t0).cross(t2 - t0);
    Eigen::Matrix<double, 2, 3> basis, coeff;
...
static EdgeEdgeDistType parallel_edge_dist_type(
    const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1) {
    ...
    if (eac != 2 || ebc != 2) {
        printf("degenerate case in parallel edges");
        std::abort();
    }
...
static double point_point_distance(const Eigen::Vector3d &p0, const Eigen::Vector3d &p1) {
    return (p0 - p1).squaredNorm();
}

static double point_edge_distance(
    const Eigen::Vector3d &p, const Eigen::Vector3d &e0, const Eigen::Vector3d &e1, PointEdgeDistType dist_type) {
...
static double point_triangle_distance_in(
    const Eigen::Vector3d &p, const Eigen::Vector3d &t0, const Eigen::Vector3d &t1, const Eigen::Vector3d &t2) {
    const Eigen::Vector3d normal = (t1 - t0).cross(t2 - t0);
...
static double edge_edge_distance_in(
    const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1) {
...
static double edge_edge_distance(
    const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1,
...
// gradient part
static Eigen::Matrix<double, 3, 2> point_point_distance_gradient(const Eigen::Vector3d &p0, const Eigen::Vector3d &p1) {
    Eigen::Matrix<double, 3, 2> gradient = Eigen::Matrix<double, 3, 2>::Zero();
...
point_line_distance_gradient(const Eigen::Vector3d &p, const Eigen::Vector3d &e0, const Eigen::Vector3d &e1) {
...
static Eigen::Matrix<double, 3, 4> line_line_distance_gradient(
    const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1) {
    Eigen::Matrix<double, 3, 4> gradient = Eigen::Matrix<double, 3, 4>::Zero();
...
static Eigen::Matrix<double, 3, 4> edge_edge_distance_gradient(
    const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1,
    EdgeEdgeDistType dist_type) {
    if (dist_type == EdgeEdgeDistType::AUTO) {
        dist_type = edge_edge_dist_type(ea0, ea1, eb0, eb1);
    ...
    case EdgeEdgeDistType::EA0_EB0: {
        const Eigen::Matrix<double, 3, 2> local_grad = point_point_distance_gradient(ea0, eb0);
        gradient.col(0) = local_grad.col(0);
        gradient.col(2) = local_grad.col(1);
        break;
    }
    case EdgeEdgeDistType::EA0_EB1: {
        const Eigen::Matrix<double, 3, 2> local_grad = point_point_distance_gradient(ea0, eb1);
        gradient.col(0) = local_grad.col(0);
        gradient.col(3) = local_grad.col(1);
        break;
    }
    case EdgeEdgeDistType::EA1_EB0: {
        const Eigen::Matrix<double, 3, 2> local_grad = point_point_distance_gradient(ea1, eb0);
        gradient.col(1) = local_grad.col(0);
        gradient.col(2) = local_grad.col(1);
        break;
    }
    case EdgeEdgeDistType::EA1_EB1: {
        const Eigen::Matrix<double, 3, 2> local_grad = point_point_distance_gradient(ea1, eb1);
        gradient.col(1) = local_grad.col(0);
        gradient.col(3) = local_grad.col(1);
        break;
    }
    case EdgeEdgeDistType::EA_EB0: {
        const Eigen::Matrix3d local_grad = point_line_distance_gradient(eb0, ea0, ea1);
    ...
    case EdgeEdgeDistType::EA_EB1: {
        const Eigen::Matrix3d local_grad = point_line_distance_gradient(eb1, ea0, ea1);
    ...
    case EdgeEdgeDistType::EA0_EB: {
        const Eigen::Matrix3d local_grad = point_line_distance_gradient(ea0, eb0, eb1);
        gradient.col(0) = local_grad.col(0);
        gradient.col(2) = local_grad.col(1);
    ...
    case EdgeEdgeDistType::EA1_EB: {
        const Eigen::Matrix3d local_grad = point_line_distance_gradient(ea1, eb0, eb1);
        gradient.col(1) = local_grad.col(0);
        gradient.col(2) = local_grad.col(1);
        gradient.col(3) = local_grad.col(2);
        break;
...
static Eigen::Matrix<double, 3, 4> point_plane_distance_gradient(
    const Eigen::Vector3d &p, const Eigen::Vector3d &t0, const Eigen::Vector3d &t1, const Eigen::Vector3d &t2) {
    Eigen::Matrix<double, 3, 4> gradient = Eigen::Matrix<double, 3, 4>::Zero();
...
static Eigen::Matrix<double, 3, 4> point_triangle_distance_gradient(
    const Eigen::Vector3d &p, const Eigen::Vector3d &t0, const Eigen::Vector3d &t1, const Eigen::Vector3d &t2,
    PointTriangleDistType dist_type) {
    Eigen::Matrix<double, 3, 4> gradient = Eigen::Matrix<double, 3, 4>::Zero();
    if (dist_type == PointTriangleDistType::AUTO) {
    ...
    case PointTriangleDistType::P_T0: {
        Eigen::Matrix<double, 3, 2> local_grad = point_point_distance_gradient(p, t0);
        gradient.col(0) = local_grad.col(0);
        gradient.col(1) = local_grad.col(1);
        break;
    }
    case PointTriangleDistType::P_T1: {
        Eigen::Matrix<double, 3, 2> local_grad = point_point_distance_gradient(p, t1);
        gradient.col(0) = local_grad.col(0);
        gradient.col(2) = local_grad.col(1);
        break;
    }
    case PointTriangleDistType::P_T2: {
        Eigen::Matrix<double, 3, 2> local_grad = point_point_distance_gradient(p, t2);
        gradient.col(0) = local_grad.col(0);
        gradient.col(3) = local_grad.col(1);
        break;
    }
    case PointTriangleDistType::P_T0T1: {
        Eigen::Matrix3d local_grad = point_line_distance_gradient(p, t0, t1);
        gradient.col(0) = local_grad.col(0);
        gradient.col(1) = local_grad.col(1);
    ...
    case PointTriangleDistType::P_T1T2: {
        Eigen::Matrix3d local_grad = point_line_distance_gradient(p, t1, t2);
        gradient.col(0) = local_grad.col(0);
        gradient.col(2) = local_grad.col(1);
        gradient.col(3) = local_grad.col(2);
        break;
    ...
    case PointTriangleDistType::P_T2T0: {
        Eigen::Matrix3d local_grad = point_line_distance_gradient(p, t2, t0);
        gradient.col(0) = local_grad.col(0);
...
// hessian part
static Eigen::Matrix<double, 6, 6> point_point_distance_hessian(const Eigen::Vector3d &p0, const Eigen::Vector3d &p1) {
    Eigen::Matrix<double, 6, 6> hessian = Eigen::Matrix<double, 6, 6>::Zero();
    hessian.diagonal().setConstant(2.0);
    for (int i = 0; i < 3; i++) { hessian(i, i + 3) = hessian(i + 3, i) = -2; }
    return hessian;
}

static Eigen::Matrix<double, 9, 9>
point_line_distance_hessian(const Eigen::Vector3d &p, const Eigen::Vector3d &e0, const Eigen::Vector3d &e1) {
...
static Eigen::Matrix<double, 12, 12> line_line_distance_hessian(
    const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1) {
...
static Eigen::Matrix<double, 12, 12> edge_edge_distance_hessian(
    const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1,
    EdgeEdgeDistType dist_type) {
    ...
    switch (dist_type) {
    case EdgeEdgeDistType::EA0_EB0: {
        Eigen::Matrix<double, 6, 6> local_hess = point_point_distance_hessian(ea0, eb0);
    ...
    case EdgeEdgeDistType::EA0_EB1: {
        Eigen::Matrix<double, 6, 6> local_hess = point_point_distance_hessian(ea0, eb1);
        hessian.topLeftCorner<3, 3>() = local_hess.topLeftCorner<3, 3>();
    ...
    case EdgeEdgeDistType::EA1_EB0: {
        Eigen::Matrix<double, 6, 6> local_hess = point_point_distance_hessian(ea1, eb0);
        hessian.block<6, 6>(3, 3) = local_hess;
        break;
    }
    case EdgeEdgeDistType::EA1_EB1: {
        Eigen::Matrix<double, 6, 6> local_hess = point_point_distance_hessian(ea1, eb1);
    ...
    case EdgeEdgeDistType::EA0_EB: {
        Eigen::Matrix<double, 9, 9> local_hess = point_line_distance_hessian(ea0, eb0, eb1);
    ...
    case EdgeEdgeDistType::EA1_EB: {
        Eigen::Matrix<double, 9, 9> local_hess = point_line_distance_hessian(ea1, eb0, eb1);
        hessian.block(3, 3, 9, 9) = local_hess;
        break;
    }
    case EdgeEdgeDistType::EA_EB0: {
        Eigen::Matrix<double, 9, 9> local_hess = point_line_distance_hessian(eb0, ea0, ea1);
    ...
    case EdgeEdgeDistType::EA_EB1: {
        Eigen::Matrix<double, 9, 9> local_hess = point_line_distance_hessian(eb1, ea0, ea1);
        hessian.block(0, 0, 6, 6) = local_hess.bottomRightCorner<6, 6>();
...
static Eigen::Matrix<double, 12, 12> point_plane_distance_hessian(
    const Eigen::Vector3d &p, const Eigen::Vector3d &t0, const Eigen::Vector3d &t1, const Eigen::Vector3d &t2) {
    Eigen::Matrix<double, 12, 12> hessian = Eigen::Matrix<double, 12, 12>::Zero();
    double v01 = p(0), v02 = p(1), v03 = p(2), v11 = t0(0), v12 = t0(1), v13 = t0(2), v21 = t1(0),
           v22 = t1(1), v23 = t1(2), v31 = t2(0), v32 = t2(1), v33 = t2(2);
...
static Eigen::Matrix<double, 12, 12> point_triangle_distance_hessian(
    const Eigen::Vector3d &p, const Eigen::Vector3d &t0, const Eigen::Vector3d &t1, const Eigen::Vector3d &t2,
    PointTriangleDistType dist_type) {
    Eigen::Matrix<double, 12, 12> hessian = Eigen::Matrix<double, 12, 12>::Zero();
    if (dist_type == PointTriangleDistType::AUTO) {
    ...
    switch (dist_type) {
    case PointTriangleDistType::P_T0: {
        Eigen::Matrix<double, 6, 6> local_hess = point_point_distance_hessian(p, t0);
        hessian.topLeftCorner<6, 6>() = local_hess;
        break;
    }
    case PointTriangleDistType::P_T1: {
        Eigen::Matrix<double, 6, 6> local_hess = point_point_distance_hessian(p, t1);
        hessian.topLeftCorner<3, 3>() = local_hess.topLeftCorner<3, 3>();
        hessian.block<3, 3>(0, 6) = local_hess.topRightCorner<3, 3>();
        hessian.block<3, 3>(6, 0) = local_hess.bottomLeftCorner<3, 3>();
        hessian.block<3, 3>(6, 6) = local_hess.bottomRightCorner<3, 3>();
    ...
    case PointTriangleDistType::P_T2: {
        Eigen::Matrix<double, 6, 6> local_hess = point_point_distance_hessian(p, t2);
        hessian.topLeftCorner<3, 3>() = local_hess.topLeftCorner<3, 3>();
        hessian.topRightCorner<3, 3>() = local_hess.topRightCorner<3, 3>();
        hessian.bottomLeftCorner<3, 3>() = local_hess.bottomLeftCorner<3, 3>();
        hessian.bottomRightCorner<3, 3>() = local_hess.bottomRightCorner<3, 3>();
    ...
    case PointTriangleDistType::P_T0T1: {
        Eigen::Matrix<double, 9, 9> local_hess = point_line_distance_hessian(p, t0, t1);
        hessian.topLeftCorner<9, 9>() = local_hess;
        break;
    }
    case PointTriangleDistType::P_T1T2: {
        Eigen::Matrix<double, 9, 9> local_hess = point_line_distance_hessian(p, t1, t2);
        hessian.topLeftCorner<3, 3>() = local_hess.topLeftCorner<3, 3>();
    ...
    case PointTriangleDistType::P_T2T0: {
        Eigen::Matrix<double, 9, 9> local_hess = point_line_distance_hessian(p, t2, t0);
        hessian.topLeftCorner<3, 3>() = local_hess.topLeftCorner<3, 3>();
