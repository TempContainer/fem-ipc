...
class EdgeEdgeCollision {
  public:
    int edge0_idx, edge1_idx;
    double eps = 0.0;

    EdgeEdgeCollision(int e0_idx, int e1_idx) : edge0_idx(e0_idx), edge1_idx(e1_idx) {}
    ...
    bool is_mollified() const { return true; }

    double mollifier(const Eigen::Matrix<double, 3, 4> &position, double eps) const {
        const Eigen::Vector3d ea0 = position.col(0), ea1 = position.col(1), eb0 = position.col(2),
                       eb1 = position.col(3);
    ...
    Eigen::Matrix<double, 3, 4> mollifier_grad(const Eigen::Matrix<double, 3, 4> &position, double eps) const {
        const Eigen::Vector3d ea0 = position.col(0), ea1 = position.col(1), eb0 = position.col(2),
                       eb1 = position.col(3);
        const double edge_edge_cross_norm_squared = edge_edge_cross_squarednorm(ea0, ea1, eb0, eb1);
    ...
    Eigen::Matrix<double, 12, 12> mollifier_hess(const Eigen::Matrix<double, 3, 4> &position, double eps) const {
        const Eigen::Vector3d ea0 = position.col(0), ea1 = position.col(1), eb0 = position.col(2),
                       eb1 = position.col(3);
        const double edge_edge_cross_norm_squared = edge_edge_cross_squarednorm(ea0, ea1, eb0, eb1);
        if (edge_edge_cross_norm_squared < eps) {
            const Eigen::Vector<double, 12> grad =
    ...
    bool isBroadIntersect(
        ...
        Eigen::Vector3d edge0_min = pos0.col(0);
        edge0_min = edge0_min.cwiseMin(pos0.col(1));
        edge0_min = edge0_min.cwiseMin(pos1.col(0));
        edge0_min = edge0_min.cwiseMin(pos1.col(1));
        Eigen::Vector3d edge0_max = pos0.col(0);
        edge0_max = edge0_max.cwiseMax(pos0.col(1));
        edge0_max = edge0_max.cwiseMax(pos1.col(0));
        edge0_max = edge0_max.cwiseMax(pos1.col(1));

        Eigen::Vector3d edge1_min = pos0.col(2);
        edge1_min = edge1_min.cwiseMin(pos0.col(3));
        edge1_min = edge1_min.cwiseMin(pos1.col(2));
        edge1_min = edge1_min.cwiseMin(pos1.col(3));
        Eigen::Vector3d edge1_max = pos0.col(2);
        edge1_max = edge1_max.cwiseMax(pos0.col(3));
        edge1_max = edge1_max.cwiseMax(pos1.col(2));
        edge1_max = edge1_max.cwiseMax(pos1.col(3));

        edge0_min.array() -= half_dHat;
        edge0_max.array() += half_dHat;
        edge1_min.array() -= half_dHat;
        edge1_max.array() += half_dHat;

        return (edge0_min.array() <= edge1_max.array()).all()
            && (edge1_min.array() <= edge0_max.array()).all();
    }

    double compute_accd_timestep(
        const Eigen::Matrix<double, 3, 4> &pos0, const Eigen::Matrix<double, 3, 4> &pos1,
        const double t_ccd_fullstep, const double thickness = 0.0, const int max_iteration = 10000) const {
        double t_ccd_addictive = 0.0;

        const Eigen::Vector3d ea0 = pos0.col(0), ea1 = pos0.col(1), eb0 = pos0.col(2), eb1 = pos0.col(3);
  ...
  private:
    ...
    double edge_edge_cross_squarednorm(
        const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1) const {
        return (ea1 - ea0).cross(eb1 - eb0).squaredNorm();
    }

    Eigen::Matrix<double, 3, 4> edge_edge_cross_squarednorm_gradient(
        const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1) const {
        Eigen::Matrix<double, 3, 4> grad = Eigen::Matrix<double, 3, 4>::Zero();
    ...
    Eigen::Matrix<double, 12, 12> edge_edge_cross_squarednorm_hessian(
        const Eigen::Vector3d &ea0, const Eigen::Vector3d &ea1, const Eigen::Vector3d &eb0, const Eigen::Vector3d &eb1) const {
        Eigen::Matrix<double, 12, 12> hess = Eigen::Matrix<double, 12, 12>::Zero();
        double v01 = ea0(0), v02 = ea0(1), v03 = ea0(2), v11 = ea1(0), v12 = ea1(1), v13 = ea1(2),
               v21 = eb0(0), v22 = eb0(1), v23 = eb0(2), v31 = eb1(0), v32 = eb1(1), v33 = eb1(2);
